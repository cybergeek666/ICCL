...
from ICCL import datasets
from ICCL import models
from ICCL.models.cm import ClusterMemory
from ICCL.trainers import Trainer
from ICCL.evaluators import Evaluator, extract_features
from ICCL.utils.data import IterLoader
from ICCL.utils.data import transforms as T
from ICCL.utils.data.sampler import RandomMultipleGallerySampler
from ICCL.utils.data.preprocessor import Preprocessor
from ICCL.utils.logging import Logger
from ICCL.utils.serialization import load_checkpoint, save_checkpoint, copy_state_dict
from ICCL.utils.faiss_rerank import compute_jaccard_distance
...
def main_worker(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, ((fname, _, cid), label) in enumerate(zip(sorted(dataset.train), pseudo_labels)):
            pid = label.item()
            if pid >= num_cluster:  # append data except outliers
                num_outliers += 1
            else:
                pseudo_labeled_dataset.append((fname, pid, cid))
                idxs.append(i)
                cids.append(cid)
                pids.append(pid)
        ...
        cluster_centers = []
        cluster2camera_centers,cluster2camera_pids,cluster2camera_cids = [],[],[]
        ...
        for pid in sorted(np.unique(pids)):  # loop all pids
            ...
            cluster_centers.append(features[idxs_p].mean(0))
            ...
            for cid in sorted(np.unique(cids[idxs_p])):  # loop all cids for pid
                ...
                cluster2camera_centers.append(features[idxs_cp].mean(0))                
                cluster2camera_pids.append(pid)
                cluster2camera_cids.append(cid)

        cluster_centers = F.normalize(torch.stack(cluster_centers), p=2, dim=1)
        memory_cluster2camera = InterCamProxy(cluster_centers.size(1), len(cluster2camera_pids)).cuda()
        memory_cluster2camera.proxy = F.normalize(torch.stack(cluster2camera_centers), p=2, dim=1).cuda()
        memory_cluster2camera.pids = torch.Tensor(cluster2camera_pids).long().cuda()
        memory_cluster2camera.cids = torch.Tensor(cluster2camera_cids).long().cuda()  
        ...
        trainer.memory_cluster2camera = memory_cluster2camera
